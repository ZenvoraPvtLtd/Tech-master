                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-zinc-800/60">
                    {formData.resumes.map((r, idx) => (
                      <tr key={r.id || idx} className="hover:bg-zinc-900/30">
                        <td className="py-2.5 px-4 font-semibold text-white">{r.name}</td>
                        <td className="py-2.5 px-4 font-mono text-luxury-gold">{r.jobTitle || 'General'}</td>
                        <td className="py-2.5 px-4 font-mono text-zinc-400">{r.email}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl text-xs">
            ...
            <div className="space-y-3">
              <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Name</span> <strong className="text-white text-sm">{selectedApplicant.name}</strong></div>
              <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Applied Position</span> <span className="text-luxury-gold font-mono">{selectedApplicant.jobTitle}</span></div>
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Email</span> <span className="text-zinc-300 font-mono">{selectedApplicant.email}</span></div>
                <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Phone</span> <span className="text-zinc-300 font-mono">{selectedApplicant.phone}</span></div>
              </div>
              {selectedApplicant.experience && (
                <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Portfolio / GitHub</span> <a href={selectedApplicant.experience} target="_blank" rel="noreferrer" className="text-luxury-gold hover:underline font-mono">{selectedApplicant.experience}</a></div>
              )}
              {selectedApplicant.message && (
                <div><span className="text-zinc-500 font-mono uppercase text-[10px] block">Why Join Answer</span> <p className="text-zinc-300 p-2 bg-zinc-900 rounded border border-zinc-800">{selectedApplicant.message}</p></div>
              )}
            </div>
